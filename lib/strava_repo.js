...
strava_repo.updateActivity = function(activity, callBack){
  activities.update({id: activity.id}, activity, function(err, result){
    if(err !== null){ logger.error('Error updating activity: ' + err.msg) }
    callBack();
  });
}

strava_repo.queryActivities = function(selection, order, callBack){
  activities.find(selection, order, function(err, activities){
